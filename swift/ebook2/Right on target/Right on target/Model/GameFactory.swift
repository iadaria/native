...
final class GameFactory {
    ...
    static func getNumericGame() -> some GameProtocol {
        ...
        let secretValue = SecretNumbericValue(initialValue: 0) { _ in
            // замыкание для выбора случайного значения
            return Array(minSecretValue...maxSecretValue).randomElement()!
        }
        return Game<SecretNumbericValue>(secretValue: secretValue, rounds: 2) { secretValue, userValue in
    ...
    static func getColorGame() -> some GameProtocol {
        ...
        return Game<SecretColorValue>(secretValue: secretValue, rounds: 2) { secretValue, userValue in
            if secretValue.value == userValue.value {
                return 1
            } else {
                return 0
